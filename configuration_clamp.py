...
class ClampTextConfig(PretrainedConfig):
    r"""
    ...
    def from_pretrained(
        ...
        if config_dict.get("model_type") == "clamp":
...
class ClampAudioConfig(PretrainedConfig):
    r"""
...
class ClampMotionConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`ClapTextModel`]. It is used to instantiate a CLAP
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the CLAP
    ...
    def __init__(
        ...
        codebook_dim=768,
        codebook_size=1024,
        codebook_sampling_temperature=0.0,
        pool_type="mean",  # max mean
        initializer_range=0.02,
        initializer_factor=1.0,
        projection_dim=512,
        classifier_dropout=None,
    ...
    def from_pretrained(
        ...
        # get the text config dict if we are loading from ClapConfig
        if config_dict.get("model_type") == "clamp":
            config_dict = config_dict["motion_config"]
...
class ClampConfig(PretrainedConfig):
    r"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if motion_config is None:
            motion_config = {}
            logger.info(
                "motion_config is None. initializing the ClapAudioConfig with default values."
            )

        self.text_config = ClampTextConfig(**text_config)
        self.audio_config = ClampAudioConfig(**audio_config)
        self.motion_config = ClampMotionConfig(**motion_config)

        self.text_config.projection_dim = projection_dim
        self.audio_config.projection_dim = projection_dim
        self.motion_config.projection_dim = projection_dim
        ...
        self.motion_config.projection_hidden_act = projection_hidden_act
    ...
    def from_text_audio_configs(
        cls,
        text_config: ClampTextConfig,
        audio_config: ClampAudioConfig,
        motion_config: ClampMotionConfig,
        **kwargs,
    ):
        r"""
    ...
    def to_dict(self):
        ...
        output["motion_config"] = self.motion_config.to_dict()
